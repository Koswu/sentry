...
class SpanDetail extends React.Component<Props, State> {
  ...
  renderRelatedSpansButton() {
    const {span, organization, location, event} = this.props;

    const spanOperation = span.op;

    if (isGapSpan(span) || !spanOperation || !span.hash) {
      return null;
    }
    ...
    const target = spanDetailsRouteWithQuery({
      ...
      spanSlug: {op: spanOperation, group: span.hash},
      projectID: event.projectID,
    });

    return (
      <StyledButton size="xsmall" to={target}>
        {t('View Related Spans')}
      </StyledButton>
    );
  }
        ...
        <SpanDetails>
          <table className="table key-value">
            <tbody>
              ...
              <Row title="Description" extra={this.renderRelatedSpansButton()}>
                {span?.description ?? ''}
              </Row>
