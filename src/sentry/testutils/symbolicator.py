...
from .cases import TransactionTestCase
...
def live_server_cls(request, live_server):
    if hasattr(request, "cls"):
        request.cls.live_server = live_server


@pytest.mark.usefixtures("live_server_cls")
class SymbolicatorTestCase(RelayStoreHelper, TransactionTestCase):
    def setUp(self):
        super().setUp()

        self.project.update_option("sentry:builtin_symbol_sources", [])
        new_prefix = self.live_server.url
    ...
    def upload_symbols(self):
        ...
        f.write(self.get_fixture_path("windows.sym"), "crash.sym")
        f.close()
    ...
    def get_fixture_path(self, name):
        return os.path.join(
            os.path.dirname(__file__), os.pardir, os.pardir, "tests", "fixtures", "native", name
        )
